...
export function createProgram(): Command {
  ...
  const stableCanary = operationTest
    .command("stable-canary")
    .description(
      "Prepare and finalize a source-free Stable install canary for Windows Sandbox."
    );

  stableCanary
    .command("prepare")
    .description(
      "Validate a published Stable verification and generate a Windows Sandbox profile."
    )
    .option(
      "--verification <path>",
      "Stable verification artifact. Defaults to the latest local PASS artifact."
    )
    .option(
      "--output <dir>",
      "Canary output directory. Defaults to .kairon/release/stable-canaries/<id>."
    )
    .requiredOption(
      "--node-runtime-root <dir>",
      "Read-only Node.js 22+ runtime directory mapped into Windows Sandbox."
    )
    .requiredOption(
      "--git-runtime-root <dir>",
      "Read-only Git runtime directory mapped into Windows Sandbox."
    )
    .option(
      "--fixture <dir>",
      "Optional read-only canary fixture. A temporary fixture is generated by default."
    )
    .option(
      "--timeout-seconds <seconds>",
      "Sandbox completion timeout recorded in the canary manifest."
    )
    .option(
      "--keep-on-failure",
      "Keep the Sandbox work directory after a failed canary."
    )
    .option(
      "--credential-provider <name>",
      "Credential provider label for evidence. Credential values are never persisted."
    )
    .option("--format <format>", "Output format: text or json.", "text")
    .action(async (options: {
      verification?: string;
      ...
      console.log(
        await prepareStableCanaryCommand(process.cwd(), options)
      );
    });
  ...
  stableCanary
    .command("finalize")
    .description(
      "Validate a Sandbox result and write a release-bound Stable canary result."
    )
    .requiredOption("--input <path>", "Generated Stable canary input.json path.")
    .option("--format <format>", "Output format: text or json.", "text")
    .action(async (options: { input?: string; format?: string }) => {
      const result = await finalizeStableCanaryCommand(
        process.cwd(),
        options
      );
      console.log(result.text);
