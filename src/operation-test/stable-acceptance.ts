...
export type StableAcceptanceCleanupPlan = {
  ...
  resources: Array<{
    ...
    cleanup_status: "not_created" | "created" | "deleted" | "verified_absent";
  }>;
  generated_at: string;
};
...
function renderStableCommandList(input: {
  ...
  const lines = [
    ...
    "}",
    "",
    "$script:STABLE_TRACKED_PROCESSES = @{}",
    "",
    "function Get-StableListeningPortOwner {",
    ...
    "    $authorizationScheme = 'Be' + 'arer'",
    "    $sanitized = [regex]::Replace($commandLine, \"(?i)$authorizationScheme\\s+\\S+\", \"$authorizationScheme [redacted]\")",
    "    $sanitized = [regex]::Replace($sanitized, '(?i)(authorization|token|password|secret)(\\s*[:=]\\s*|\\s+)\\S+', '$1=[redacted]')",
    "    if ($sanitized.Length -gt 240) { $sanitized = $sanitized.Substring(0, 240) + \"...\" }",
    "    [pscustomobject]@{ port=$Port; process_id=$ownerPid; command_line=$sanitized }",
    "  }",
    "}",
    "",
    "function Assert-StablePortAvailable {",
    "  param([Parameter(Mandatory=$true)][int]$Port)",
    "  $owners = @(Get-StableListeningPortOwner -Port $Port)",
    "  if ($owners.Count -eq 0) { return }",
    "  $details = @($owners | ForEach-Object { \"pid=$($_.process_id) command=$($_.command_line)\" }) -join '; '",
    "  throw \"Stable acceptance port $Port is already in use: $details\"",
    "}",
    "",
    "function Set-StableTrackedProcessCleanupStatus {",
    "  param(",
    "    [Parameter(Mandatory=$true)][int]$ProcessId,",
    "    [Parameter(Mandatory=$true)][ValidateSet(\"created\",\"deleted\",\"verified_absent\")][string]$Status",
    "  )",
    "  $cleanup = Get-Content $STABLE_CLEANUP_PLAN -Raw -Encoding UTF8 | ConvertFrom-Json",
    "  $resource = @($cleanup.resources | Where-Object { $_.alias -eq \"TEST_TUNNEL_PROCESS\" })[0]",
    "  if ($null -eq $resource) { throw \"TEST_TUNNEL_PROCESS cleanup resource is missing\" }",
    "  $resource.exact_id = \"pid:$ProcessId\"",
    "  $resource.cleanup_status = $Status",
    "  $json = $cleanup | ConvertTo-Json -Depth 30",
    "  [IO.File]::WriteAllText($STABLE_CLEANUP_PLAN, $json + [Environment]::NewLine, [Text.UTF8Encoding]::new($false))",
    "}",
    "",
    "function Start-StableTrackedProcess {",
    "  param(",
    ...
    "  )",
    "  Assert-StablePortAvailable -Port $Port",
    "  $started = Start-Process -FilePath $FilePath -ArgumentList $ArgumentList -PassThru -WindowStyle Hidden",
    "  try {",
    "    $owner = $null",
    "    for ($attempt = 0; $attempt -lt 50; $attempt++) {",
    "      if ($started.HasExited) { throw \"tracked process exited before listening on port $Port\" }",
    "      $owner = Get-CimInstance Win32_Process -Filter \"ProcessId = $($started.Id)\" -ErrorAction SilentlyContinue",
    "      $listening = @(Get-StableListeningPortOwner -Port $Port | Where-Object { $_.process_id -eq $started.Id })",
    "      if ($null -ne $owner -and $listening.Count -eq 1) { break }",
    "      Start-Sleep -Milliseconds 100",
    "    }",
    "    if ($null -eq $owner) { throw \"tracked process identity is unavailable\" }",
    "    $listening = @(Get-StableListeningPortOwner -Port $Port | Where-Object { $_.process_id -eq $started.Id })",
    "    if ($listening.Count -ne 1) { throw \"tracked process did not own port $Port\" }",
    "    $script:STABLE_TRACKED_PROCESSES[$Alias] = [pscustomobject]@{",
    "      process_id = [int]$started.Id",
    "      creation_date = [string]$owner.CreationDate",
    "      command_line = [string]$owner.CommandLine",
    "      port = $Port",
    "    }",
    "    Set-StableTrackedProcessCleanupStatus -ProcessId $started.Id -Status created",
    "    return $started",
    "  } catch {",
    "    if (-not $started.HasExited) { Stop-Process -Id $started.Id -Force -ErrorAction SilentlyContinue }",
    "    Set-StableTrackedProcessCleanupStatus -ProcessId $started.Id -Status deleted",
    "    throw",
    "  }",
    "}",
    "",
    "function Stop-StableTrackedProcess {",
    "  param([Parameter(Mandatory=$true)][string]$Alias)",
    "  $tracked = $script:STABLE_TRACKED_PROCESSES[$Alias]",
    "  if ($null -eq $tracked) { throw \"Stable acceptance process is not tracked: $Alias\" }",
    "  $current = Get-CimInstance Win32_Process -Filter \"ProcessId = $($tracked.process_id)\" -ErrorAction SilentlyContinue",
    "  if ($null -eq $current) {",
    "    Set-StableTrackedProcessCleanupStatus -ProcessId $tracked.process_id -Status verified_absent",
    "    $script:STABLE_TRACKED_PROCESSES.Remove($Alias)",
    "    return",
    "  }",
    "  if ([string]$current.CreationDate -ne $tracked.creation_date -or [string]$current.CommandLine -ne $tracked.command_line) {",
    "    throw \"Refusing to stop process $($tracked.process_id): tracked identity no longer matches\"",
    "  }",
    "  Stop-Process -Id $tracked.process_id -Force -ErrorAction Stop",
    "  Set-StableTrackedProcessCleanupStatus -ProcessId $tracked.process_id -Status deleted",
    "  $script:STABLE_TRACKED_PROCESSES.Remove($Alias)",
    "}",
    "",
    ...
    "```",
    "",
    "<!-- command-group: LIVE_PROCESS_PREFLIGHT -->",
    "```powershell",
    "# Board / Discord / proxyを起動する前に実行する。unknown ownerは自動停止しない。",
